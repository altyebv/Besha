...
/**
 * Manages streak calculation and daily activity tracking.
 * 
 * Streak Rules:
 * - FLAME days grow the streak count
 * - SPARK days maintain the streak (don't break it, but don't add to count)
 * - COLD days (or missing days) break the streak
 * - Streaks are calculated from consecutive days of activity (FLAME or SPARK)
 */
@Singleton
class StreakManager @Inject constructor(
    ...
    fun getTodayDate(): String = LocalDate.now().format(DATE_FORMAT)
    
    fun getYesterdayDate(): String = LocalDate.now().minusDays(1).format(DATE_FORMAT)
    ...
    /**
     * Returns the streak status as a Flow for reactive UI.
     */
    ...
    private fun calculateCurrentStreak(activities: List<DailyActivityEntity>, today: String): Int {
        ...
        while (true) {
            ...
            when (activity?.streakLevel) {
                ...
                StreakLevel.SPARK -> {
                    // Spark maintains streak but doesn't add to count
                    // However, for display purposes, we count it as a day
                    streak++
                }
                else -> {
        ...
        // Return flame days as the "real" streak (or total if you prefer)
        // For now, return total consecutive days
        return streak
    }
